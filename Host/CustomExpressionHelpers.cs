...
internal static class CustomExpressionHelpers
{
    private static readonly MethodInfo _createAndConvert = typeof(CustomExpressionHelpers)
        ...
        bool forward)
    {
        ...
        for (var i = 0; i < keys.Length; i++)
        {
            ...
            for (var j = 0; j < handled.Count; j++)
            {
                ...
                //Proof of concept handling of handled expressions
                {
                    keyExpr = Expression.Equal(
                        Expression.Convert(ReplaceParameter(handledKey.Expression, parameter), typeof(int)),
                        Expression.Convert(cursorExpr[j], typeof(int))
                    );

                    current = current is null ? keyExpr : Expression.AndAlso(current, keyExpr);
                }
                // Default handling of handled expressions
                // {
                //     keyExpr = Expression.Equal(
            ...
            //Custom cursor proof of concept enum handling
            if (key.Expression.ReturnType.BaseType == typeof(Enum))
            {
                keyExpr = greaterThan
                    ? Expression.GreaterThan(
                        Expression.Convert(ReplaceParameter(key.Expression, parameter),
                            typeof(int)),
                        Expression.Convert(cursorExpr[i], typeof(int)) 
                    )
                    : Expression.LessThan(
                        Expression.Convert(ReplaceParameter(key.Expression, parameter), typeof(int)),
                        Expression.Convert(cursorExpr[i], typeof(int))
                    );
            ...
            {
                //CompareTo of IComparable cannot be translated to db side
                //MethodInfo customCompareTo = typeof(SomeEnum).GetMethod("CompareTo")!;
                
                //Default handling of cursor keys
                keyExpr = greaterThan
                    ? Expression.GreaterThan(
